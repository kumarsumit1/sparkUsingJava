package com.spark.java.streaming;
...
import java.util.*;
/**
 * https://github.com/zdata-inc/SparkSampleProject
...
public class SentimentAnalysis {
	...
	private static final String KAFKA_TOPIC = Properties
			.getString("rts.spark.kafka_topic");
	private static final int KAFKA_PARALLELIZATION = Properties
			.getInt("rts.spark.kafka_parallelization");
	...
	public static void main(String[] args) {
		...
		JavaPairReceiverInputDStream<String, String> messages = KafkaUtils.createStream(ssc, Properties.getString("rts.spark.zkhosts"),"twitter.sentimentanalysis.kafka", topicMap);

		JavaDStream<String> json = messages
		...
		JavaPairDStream<Long, String> tweets = json
		...
		JavaDStream<Tuple2<Long, String>> tweetsFiltered = filtered
				.map(new TextFilterFunction());

		tweetsFiltered = tweetsFiltered.map(new StemmingFunction());

		JavaPairDStream<Tuple2<Long, String>, Float> positiveTweets = tweetsFiltered
				.mapToPair(new PositiveScoreFunction());

		JavaPairDStream<Tuple2<Long, String>, Float> negativeTweets = tweetsFiltered
				.mapToPair(new NegativeScoreFunction());
		...
		JavaDStream<Tuple4<Long, String, Float, Float>> scoredTweets = joined
				.map(new Function<Tuple2<Tuple2<Long, String>, Tuple2<Float, Float>>, Tuple4<Long, String, Float, Float>>() {
					...
					@Override
					public Tuple4<Long, String, Float, Float> call(	Tuple2<Tuple2<Long, String>, Tuple2<Float, Float>> tweet) {
		...
		JavaDStream<Tuple5<Long, String, Float, Float, String>> result = scoredTweets
